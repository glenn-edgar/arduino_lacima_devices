...
public:
   ...
   bool select(const int db );
     ...
     bool ltrim(const char* key,const int start, const int stop );

    bool lpush(const char *key,const char *value );
    
    bool job_queue_push(const char *key ,const char *value, int limit );
    
    String job_queue_pop(const char *key  );
    
    bool stream_add_capped(const char *key, const char *value,int max_length);


    bool stream_add(const char *key, const char *value);
